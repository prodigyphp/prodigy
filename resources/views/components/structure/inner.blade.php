...
<div class="inner {{ ($editing) ? 'pro-group pro-relative' : '' }}" style="{{ $styles }}">
    @if($editing)
        ...
        <div class="pro-absolute pro-bg-blue-500 pro-text-white pro-gap-2 pro-hidden group-hover:pro-flex" style="z-index: 999;">
            <button class="pro-px-2 pro-py-1 pro-text-sm" wire:click="$emit('editBlock', {{ $block->id }})">Edit</button>
            <button class="pro-px-2 pro-py-1 pro-text-sm" wire:click="$emit('duplicateBlock', {{ $block->id }})">Duplicate</button>
            <button class="pro-px-2 pro-py-1 pro-text-sm" wire:click="$emit('deleteBlock', {{ $block->id }})">Delete</button>
        </div>
    @endif
